#ifndef _BASE_LOCK_RWLOCK_IMPL_H_
#define _BASE_LOCK_RWLOCK_IMPL_H_
...
namespace base {
    ...
    namespace lock {
        inline RWLock::RWLock(bool writeFirst) :
        ...
        inline int RWLock::writeLock() {
            ...
            if (std::this_thread::get_id() != this->m_write_thread_id) {
                ...
                for (int zero = FREE_STATUS;
                     !this->m_lockCount.compare_exchange_weak(zero, WRITE_LOCK_STATUS);
                     zero = FREE_STATUS);
...
#endif /* end of include guard: _BASE_LOCK_RWLOCK_IMPL_H_ */
