//
// Created by carl on 17-9-27.
//

#ifndef _BASE_RAII_HPP_
#define _BASE_RAII_HPP_
...
namespace base {
    ...
    public:
        ...
        typename std::enable_if<std::is_pointer<_T>::value, _T>::type operator->() const noexcept { return _resource; }

        template<typename _T=T>
        typename std::enable_if<std::is_class<_T>::value, _T *>::type
        operator->() const noexcept { return std::addressof(_resource); }
