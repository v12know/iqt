...
class EventBus(object):
    ...
    def add_listener(self, event_type, listener, target_id=None):
        if event_type in [EVENT.ORDER_PENDING_NEW, EVENT.ORDER_CREATION_PASS, EVENT.ORDER_CREATION_REJECT,
                          EVENT.ORDER_PENDING_CANCEL, EVENT.ORDER_CANCELLATION_PASS, EVENT.ORDER_CANCELLATION_REJECT,
                          EVENT.ORDER_UNSOLICITED_UPDATE]:
            iqt.EventBus.add_listener_order(event_type.value, listener, target_id)
        else:
    ...
    def publish_event(self, event):
        if event.event_type in [EVENT.POST_UNIVERSE_CHANGED]:
            iqt.EventBus.publish_event_universe(event.event_type.value, event.universe)
        else:
